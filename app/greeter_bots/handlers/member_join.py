...
def get_router(bot_id: int) -> Router:
    ...
    asyncio.create_task(send_log_to_admin(f"📌 Greeter router инициализирован для bot_id={bot_id}"))
    ...
    async def handle_member_join(update: ChatMemberUpdated):
        if (
            ...
            await redis.rpush("join_queue", json.dumps({
                "user_id": user_id,
                "chat_id": chat_id,
                "bot_id": bot_id
            }))
